...
;Class#: 12379
;Description: This code continuously outputs the character U to the terminal via USART and allows for external probbing via PORTC 
;
...
TX_POLL:
	lds R17, USARTC0_STATUS		;Check the status register for USARTD0 to see if the data register empty flag was set
	sbrs R17, 5					
...
RX_POLL:
	lds R16, USARTC0_STATUS		;Check the status register for the receive complete flag and sees if it is set
	sbrs R16, 7					
	rjmp RX_POLL				;else continue polling
	lds R16, USARTC0_DATA		;read the character into R16
...
INIT_USART:
	...
	ldi R16, 0b00100011
	sts USARTC0_CTRLC, R16		;Set Parity to even, 8 bit frame, 1 stop bit
...
; FUNCTION:     Set up PortC_PIN3 as output for TX pin 
;               of USARTD0 using the pins alternate functions and initial output value to 1.
;				Then set up the PORTC_PIN4 as input for RX pin of USARTD0.
INIT_GPIO:
	ldi R16, 0x08	
	sts PortC_OUTSET, R16	;set the TX line to default to '1' as 
							;  described in the documentation
	sts PortC_DIRSET, R16	;Must set PortD_PIN3 as output for TX pin 
							;  of USARTD0					
	ldi R16, 0x04
