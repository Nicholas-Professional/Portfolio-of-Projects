...
;Class#: 12379
;Description: This code uses an interrupt to check when the receive complete flag is set and then takes in the character and outputs it to the terminal
 */ 
...
;When the receive complete interrupt is triggered jump to the designated ISR 
...
;Initialize Y to point to the beginning of the data space allocated for the input string 
	...
	out CPU_RAMPY, r16
	;Set up the green light for infinite loop to toggle 
	ldi r16, 0b00100000
...
;Checks for backspace if backspace is found then decrement Y 
	cpi r16,0x08
	breq DELETE
;Same with the backspace but for the delete key, 
	cpi r16, 0x7F
	breq DELETE
...
INIT_USART:
	...
	;Initialize the interrupt as low priority then enable global interrupts of low priority level 
	ldi r16, 0b00010000
...
;************************************************
; ISR:   READ_ISR
; FUNCTION:     Loads in the data that typed in the console and outputs it to the terminal before returning to the main routine again 
READ_ISR:
;Retain the status register 
in r18, CPU_SREG
push r18
;Take the data from the terminal and display it 
